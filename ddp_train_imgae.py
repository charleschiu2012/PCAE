...
from PCAE.models import ImgDecoder, LMImgEncoder, ImgAE
from PCAE.loss import ssim_loss
...
parser.add_argument('--resample_amount', type=int,
...
parser.add_argument('--prior_epoch', type=str,
                    help='Which epoch of autoencoder to train to ImgEncoder')
parser.add_argument('--img_encoder_epoch', type=str,
                    help='Which epoch of ImgEncoder')
parser.add_argument('--loss_scale_factor', type=int,
...
class LMTrainSession(Network):
    def __init__(self, dataloader, model=None, sampler=None):
        ...
        self.avg_mse_step_loss = .0
        self.avg_mse_epoch_loss = .0
        self.avg_ssim_step_loss = .0
        self.avg_ssim_epoch_loss = .0
        self.data_length = 0
    ...
    def train(self):
        ...
        for epoch_idx in range(self._epoch - 1, config.network.epoch_num):
            ...
            for idx, (inputs_img, inputs_pc, targets, _, _) in enumerate(self.get_data()):
                ...
                re_imgs = self.model(inputs_img)
                mse_loss = torch.nn.MSELoss()(re_imgs, inputs_img)
                _ssim_loss = ssim_loss(re_imgs, inputs_img)
                loss = mse_loss + _ssim_loss

                loss.backward()
                self.optimizer.step()
                self.log_step_loss(sum_loss=loss.item() * len(inputs_img),
                                   mse_loss=mse_loss.item() * len(inputs_img),
                                   _ssim_loss=_ssim_loss.item() * len(inputs_img),
                                   step_idx=idx + 1)
    ...
    def set_model(self):
        self.model = ImgAE(latent_size=config.network.latent_size, image_shape=128)
    ...
    def log_step_loss(self, sum_loss, mse_loss, _ssim_loss, step_idx):
        self.avg_step_loss += sum_loss
        self.avg_epoch_loss += sum_loss
        self.avg_mse_step_loss += mse_loss
        self.avg_mse_epoch_loss += mse_loss
        self.avg_ssim_step_loss += _ssim_loss
        self.avg_ssim_epoch_loss += _ssim_loss
        ...
        if step_idx % config.wandb.step_loss_freq == 0:
            ...
            if ((argument.local_rank is not None) and config.cuda.rank[0] == 0) and config.wandb.visual_flag:
                self.visualizer.log_step_loss(step_idx=step_idx, step_loss=self.avg_step_loss, loss_name='SUM')
                self.visualizer.log_step_loss(step_idx=step_idx, step_loss=self.avg_mse_step_loss, loss_name='MSE')
                self.visualizer.log_step_loss(step_idx=step_idx, step_loss=self.avg_ssim_step_loss, loss_name='SSIM')
            elif (argument.local_rank is None) and config.wandb.visual_flag:
                self.visualizer.log_step_loss(step_idx=step_idx, step_loss=self.avg_step_loss, loss_name='SUM')
                self.visualizer.log_step_loss(step_idx=step_idx, step_loss=self.avg_mse_step_loss, loss_name='MSE')
                self.visualizer.log_step_loss(step_idx=step_idx, step_loss=self.avg_ssim_step_loss, loss_name='SSIM')
    ...
    def log_epoch_loss(self):
        ...
        self.avg_mse_epoch_loss /= self.data_length
        self.avg_ssim_epoch_loss /= self.data_length
        ...
        if ((argument.local_rank is not None) and config.cuda.rank[0] == 0) and config.wandb.visual_flag:
            ...
            self.visualizer.log_epoch_loss(epoch_idx=self._epoch, loss_name='MSE',
                                           train_epoch_loss=self.avg_mse_epoch_loss)
            self.visualizer.log_epoch_loss(epoch_idx=self._epoch, loss_name='SSIM',
                                           train_epoch_loss=self.avg_ssim_epoch_loss)
        elif (argument.local_rank is None) and config.wandb.visual_flag:
            self.visualizer.log_epoch_loss(epoch_idx=self._epoch, loss_name='SUM',
                                           train_epoch_loss=self.avg_epoch_loss)
            self.visualizer.log_epoch_loss(epoch_idx=self._epoch, loss_name='MSE',
