...
class ImgNICEValidSession(Network):
    ...
    def validate(self):
        ...
        for i, model_path in enumerate(self.models_path):
            ...
            final_step = 0
            with torch.no_grad():
                for idx, (inputs_imgs, inputs_pc, targets, _, _) in enumerate(self.get_data()):
                    final_step = idx

                    all_view_latent_imgs = self.model(inputs_imgs)
                    latent_imgs_list = torch.split(all_view_latent_imgs, 24, dim=0)
                    latent_imgs = torch.cat([latent_imgs_list[idx].mean(dim=0, keepdim=True)
                                             for idx in range(len(latent_imgs_list))])
